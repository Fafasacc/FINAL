...
// Fetch user details from the frontend (userName passed via GET parameter)
$userName = isset($_GET['userName']) ? $_GET['userName'] : '';  // Get userName from GET parameter

// Sanitize user input to prevent SQL injection
$userName = $conn->real_escape_string(strtolower($userName));  // Convert name to lowercase for case-insensitive matching
...
$sql = "
    ...
    WHERE LOWER(c.OWNER_NAME) = ?  -- Case-insensitive match for OWNER_NAME
";

// Apply search filter if provided
if ($searchTerm) {
    $sql .= " AND (LOWER(p.PETS_NAME) LIKE ?)";
}
...
// Bind parameters
if ($searchTerm) {
    $stmt->bind_param("ss", $userName, $searchTerm);  // Bind userName and search term
} else {
